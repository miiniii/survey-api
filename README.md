...
### 캐시 스케일링 전략
- 단일 노드에서 100MB 한도에 도달하면 LRU 퇴출이 발생했기 때문에, 이를 확장하기 위해 **애플리케이션 레벨 샤딩(3노드)** 을 구성해 성능을 검증했다.  
<br>

#### 왜 Redis Cluster가 아닌 애플리케이션 레벨 샤딩인가?
- **실험 목적**: 포트폴리오와 장애 대응 시나리오에 초점을 맞춰,  
  복잡한 운영 자동화보다 **샤딩 동작 원리 자체를 직접 검증**하는 것이 목적이었음.  
- **Redis Cluster 단점**: 슬롯 관리·재배치·운영 난이도가 높아, 단순 실험 환경에서는 과도한 오버헤드.  
- **애플리케이션 샤딩 장점**: 코드 단에서 샤드 노드를 직접 지정해 **구현·관찰이 단순**하고,노드 1대 장애 시 영향을 쉽게 확인할 수 있음.  
